...
CONTAINS
   ...
   SUBROUTINE p4z_sink ( kt, knt )
     ...
     call zz_sinking_advection(zz_Pmicro, zz_Pmicro_w_sink, zz_Pmicro_new,zz_alpha_b_Si,jpdia)
     call zz_sinking_advection(zz_D_PON, zz_w_sink_D_PON*zz_unvec, zz_D_PON_new,zz_alpha_b_N,jppon)
     call zz_sinking_advection(zz_D_bSi, zz_w_sink_D_bSi*zz_unvec, zz_D_bSi_new,zz_alpha_b_Si,jpdsi)
     call zz_sinking_advection(trb(:,:,:,jptra), wsink*zz_unvec, zz_TRA_new,zz_alpha_b_T,jptra)
     WHERE (tmask==1)
         tra(:,:,:,jpdia)=tra(:,:,:,jpdia)+zz_Pmicro_new
         tra(:,:,:,jppon)=tra(:,:,:,jppon)+zz_D_PON_new
         tra(:,:,:,jpdsi)=tra(:,:,:,jpdsi)+zz_D_bSi_new
         tra(:,:,:,jptra)=tra(:,:,:,jptra)+zz_TRA_new
     END WHERE
     IF( l_trdtrc ) THEN
         strsms="TBS_"
   ...
   END SUBROUTINE p4z_sink
     ...
     subroutine zz_sinking_advection(zz_qty, zz_w_sink, zz_qty_new, zz_alpha_b,jn) !zz_dz, zz_dt, 
    ...
    !real(kind=wp), dimension(jpi,jpj,0:jpk) :: zz_bmask ! positive is upward
    real(kind=wp), dimension(jpi,jpj) :: zz_bflux  ! positive is upward
    ...
    zz_flux(:,:,0) = 0._wp ! no flux in or out from above
         ...
         DO jj = 1, jpj
            DO ji = 1, jpi
               if (zz_w_sink(ji,jj,jk).lt.0) then
                  ...
                  !zz_bmask(ji,jj,jk)=tmask(ji,jj,jk)-tmask(ji,jj,jk+1)
               endif
            END DO
         END DO
      END DO
      ...
      DO jk = 1,jpkm1
         DO jj = 1, jpj
            DO ji = 1, jpi
                zz_qty_new(ji,jj,jk) = -1._wp * (zz_flux(ji,jj,jk-1) - zz_flux(ji,jj,jk))/fse3t(ji,jj,jk)
            END DO
         END DO
      END DO
      IF( l_trdtrc) THEN
         CALL  trd_flx( zz_flux(:,:,0:(jpk-1)), jn, "BSZ_","W" )
         DO jj = 1, jpj
            DO ji = 1, jpi
              zz_bflux(ji,jj)=zz_flux(ji,jj,mbkt(ji,jj))
            END DO
         END DO
         CALL trd_flx_2d(zz_bflux(:,:), jn,"BFX_","W")
      ENDIF
   ...
   SUBROUTINE p4z_sink_init
      ...
      NAMELIST/nampissink/ zz_w_sink_Pmicro_min, zz_w_sink_Pmicro_max, zz_w_sink_D_PON, zz_w_sink_D_bSi, &
                           & zz_alpha_b_Si, zz_alpha_b_N, zz_alpha_b_T
