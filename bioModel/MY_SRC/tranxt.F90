MODULE tranxt
   ...
   !!                    2015 (E. Olson) adapted to include adjustment of river
   !!                          fluxes for SMELT configuration
   !!----------------------------------------------------------------------
...
CONTAINS
   ...
   SUBROUTINE tra_nxt_vvl( kt, kit000, p2dt, cdtype, ptb, ptn, pta, psbc_tc, psbc_tc_b, kjpt )
      ...
      ELSE                          
         ...
         ll_rnf     = ln_rnf           !  .FALSE.          ! passive tracers or NO river runoffs
         ...
         DO jk = 1, jpkm1
            ...
            DO jj = 1, jpj
               DO ji = 1, jpi
                  ...
                  ! river runoff
                  IF( ll_rnf .AND. jk <= nk_rnf(ji,jj) .AND. ( cdtype == 'TRA' ))                                          &
                     &     ztc_f  = ztc_f  - zfact1 * ( rnf_tsc(ji,jj,jn) - rnf_tsc_b(ji,jj,jn) ) & 
                     &                              * fse3t_n(ji,jj,jk) / h_rnf(ji,jj)
#if defined key_pisces
                  ! river runoff for passive tracers
                  IF( ll_rnf .AND. jk <= nk_rnf(ji,jj) .AND. ( cdtype == 'TRC' ))                                          &
                     &     ztc_f  = ztc_f  - zfact1 * ( rnf_pis(ji,jj,jn) - rnf_pis_b(ji,jj,jn) ) & 
                     &                              * fse3t_n(ji,jj,jk) / h_rnf(ji,jj)
#endif
                  ! ice shelf
                  IF( ll_isf ) THEN
