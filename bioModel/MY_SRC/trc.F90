...
#if defined key_bdy
      IF ( lk_bdy ) THEN
         ALLOCATE( cn_trc_dflt(nb_bdy)  , cn_trc(nb_bdy)          ,   &
         &         nn_trcdmp_bdy(nb_bdy), trcdta_bdy(jptra,nb_bdy),   &
         &         STAT = trc_alloc  )
         IF( trc_alloc /= 0 )   CALL ctl_warn('trc_alloc: failed to allocate arrays (lk_bdy=T)')
      ENDIF
#endif
      !
