        ...
        with nc.Dataset(flist['ptrc_T']['paths'][ii]) as fp, \
            ...
            for var in varlist:
                for ipath in vlines.keys():
                    ...
                    e3v_x=e3v[:,:,j,i0:i1]
                    e1v_x=e1v[:,j,i0:i1]
                    no3_x=np.mean(np.ma.masked_where(tmask24[:,:,j:(j+2),i0:i1]==0,fp.variables[var][:,:,j:(j+2),i0:i1]),2)
                    no3T_x=v_x*e3v_x*e1v_x*no3_x*vmask[:,:,j,i0:i1]
                    Tr[var][ipath][ii]=np.sum(no3T_x)
                for ipath in ulines.keys():
                    i=ulines[ipath]['i'];j0=ulines[ipath]['j'][0];j1=ulines[ipath]['j'][1]
                    if not ipath in Tr[var].keys():
                        Tr[var][ipath]=np.zeros((len(flist['ptrc_T']['paths']),))
                    ...
                    e3u_x=e3u[:,:,j0:j1,i]
                    e2u_x=e2u[:,j0:j1,i]
                    no3_x=np.mean(np.ma.masked_where(tmask24[:,:,j0:j1,i:(i+2)]==0,fp.variables[var][:,:,j0:j1,i:(i+2)]),3)
...
if __name__ == "__main__":
    ...
    mod_end=dt.datetime(2015,1,31)
    savepath='../../save/transpLinesAllVars'+mod_start.strftime(fformat0)+'-'+mod_end.strftime(fformat0)+'.pkl'
